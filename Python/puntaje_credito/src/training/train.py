...
class CreditScoringModelTraining:
    def __init__(self, config_path: Path) -> None:
        with open(config_path, 'r') as f: #Abrir el archivo YAML que esta en la ruta
            self.params = yaml.safe_load(f)
        
        log.info(f"--- Configuracion del Entrenamiento ---")
        # paths
        self.dataset_path = Path(self.params['data_source']['data_path']['dataset_path']) #Rura CSV
        self.artifact_name_or_path = self.params['data_source']['data_path']['artifact_path'] #Nombre artefacto = model
        self.preprocessor_filename = self.params['data_source']['data_path']['preprocessor_filename'] #El archivo .joblib
    ...
    def _log_model_with_signature(self, model: nn.Module, x_example: torch.Tensor):
        ...
        pip_requirements = [
            ...
            # "torch==2.7.1+cu126",
            "torch==2.10.0",  # fallback genérico para PyPI, 2.7.1 cambiado por 2.10.0
            "scikit-learn",
            "pandas",
            "numpy",
        ]

        # Suprimir warning de PyTorch sobre buffers no escribibles durante la exportación
...
"""
ejecutar archivo de entrenamiento:
